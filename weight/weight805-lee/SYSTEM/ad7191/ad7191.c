...
//	static int sum_s= 0;
	...
	static int j1=0;
	static int  d =0 ;
	static int data1_max[16] ,data1_min[16] ;	//极值数组
	...
	{	
		...
		{	
			for(pre_m= 0;pre_m<N-1;pre_m++)
			
			{	for(pre_n=0;pre_n<N-pre_m-1;pre_n++)
				{
		...
		sum_sum = 0;
		data_sum =0;
		memset(data1_max, 0, sizeof(int) * N);
		memset(data1_min, 0, sizeof(int) * N);
	}
		return amp_v;
}
