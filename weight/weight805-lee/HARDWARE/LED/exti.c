...
Quality_Area quality_area;	//是否需要静态变量/							/*重量如何获取*/
/*********************************************************************************/
...
/**************************************************修改前后*****************************************************/
			//波动标志位
			if((amp11_t>2)&&(amp11_t<shang_old))//
			{
				over_top = 1;
				//添加下限次数
				WRITE_HMI_Data(0x2111,++(quantity_area.quantity_area1));//写入串口屏
				delay_ms(10);
				quality_area.quality_area1+= amp11_t;
				WRITE_HMI_Data(0x2211,quality_area.quality_area1*10);
				delay_ms(10);
			}
			if(amp11_t>xia_old)
			{
				below_bottom  = 1;
				//添加上限次数
				WRITE_HMI_Data(0x2511,++(quantity_area.quantity_area3));//写入串口屏
				delay_ms(10);
				quality_area.quality_area3 += amp11_t;
				WRITE_HMI_Data(0x2611,quality_area.quality_area3*10);
				delay_ms(10);
			}
			if((amp11_t<xia_old)&&(amp11_t>shang_old))
			{
				//添加合格次数
				WRITE_HMI_Data(0x2311,++(quantity_area.quantity_area2));//写入串口屏
				delay_ms(10);
				quality_area.quality_area2 += amp11_t;
				WRITE_HMI_Data(0x2411,quality_area.quality_area2*10);
				delay_ms(10);
			}
/**************************************************************************************************************/			
			flag_moving=0;
