...
/***************************************修改后*******************************************************/
		...
		if(str_compare(&Rx_Deal_Str[cnt],command_12,6)==STR_EQUALS )/*编号*/
		{
			...
			widget_addr.name_addr = (num_product-1)*8+0x1140;	//品名地址
			widget_addr.min_addr = (num_product-1)*8+0x1140+2;	//下限地址
			widget_addr.max_addr = (num_product-1)*8+0x1140+4;	//上限地址
			widget_addr.corr_addr = (num_product-1)*8+0x1140+6;	//修正系数地址
			...
			if(num_product<26)
			{
				READ_HMI(widget_addr.name_addr,0x02);	//品名
				READ_HMI(widget_addr.min_addr,0x02);	//读取下限
				READ_HMI(widget_addr.max_addr,0x02);	//读取上限
				READ_HMI(widget_addr.corr_addr,0x02);	//读取修正系数
			}
		}
